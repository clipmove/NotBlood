...
int powerupCheck(PLAYER *pPlayer, int nPowerUp)
{
    ...
    if (gNoTarget && (nPowerUp == kPwUpShadowCloak)) // no target cheat
...
static void playerResetTeamId(int nPlayer, int bNewLevel)
{
    ...
    if ((gGameOptions.nGameType != kGameTypeTeams) && (gProfile[nPlayer].nColorPreference > 0) && !(gGameOptions.uNetGameFlags&kNetGameFlagVanillaColors))
...
void playerStart(int nPlayer, int bNewLevel)
{
    ...
    if (!VanillaMode() && (gGameOptions.nGameType == kGameTypeTeams) && !(gGameOptions.uNetGameFlags&kNetGameFlagNoTeamColors) && (gGameOptions.uNetGameFlags&kNetGameFlagCalebOnly || !gProfile[pPlayer->nPlayer].nModel))
        pSprite->pal = playerColorPalSprite(pPlayer->teamIdPal);
    else
        pSprite->pal = playerColorPalDefault(pPlayer->teamIdPal);
...
char PickupAmmo(PLAYER* pPlayer, spritetype* pAmmo) {
    ...
    if (gGameOptions.nAmmoScale && !VanillaMode())
    {
        ...
        {
        case 3: // 0.25x
            pPlayer->ammoCount[nAmmoType] = ClipHigh(oldAmmoCount + (nAmmoAdded>>2), gAmmoInfo[nAmmoType].max);
            break;
        case 4: // 0.5x
            pPlayer->ammoCount[nAmmoType] = ClipHigh(oldAmmoCount + (nAmmoAdded>>1), gAmmoInfo[nAmmoType].max);
            break;
        case 5: // 0.75x
            pPlayer->ammoCount[nAmmoType] = ClipHigh(oldAmmoCount + ((nAmmoAdded>>1)+(nAmmoAdded>>2)), gAmmoInfo[nAmmoType].max);
            break;
        case 1: // 1.25x
            pPlayer->ammoCount[nAmmoType] = ClipHigh(oldAmmoCount + nAmmoAdded+(nAmmoAdded>>2), gAmmoInfo[nAmmoType].max);
            break;
        case 2: // 1.5x
            pPlayer->ammoCount[nAmmoType] = ClipHigh(oldAmmoCount + nAmmoAdded+(nAmmoAdded>>1), gAmmoInfo[nAmmoType].max);
            break;
        }
    }
    return 1;
...
void PickUp(PLAYER *pPlayer, spritetype *pSprite)
{
    ...
    if (pPlayer == gMe && showMsg)
    {
        ...
        else if ((nType == kItemShroomDelirium) && (pSprite->cstat&CSTAT_SPRITE_INVISIBLE) && !VanillaMode()) return; // do not print text for invisible delirium shrooms
            ...
            if (bKillerSpreeActive && bKillerAlive) // if killed enemy within multi kill time window, reward point
            {
                ...
                if (gMultiKill && (pKiller != gMe) && ((gMultiKillsFrags[nKiller] % 5) == 0)) // announce killing spree every 5 kills
                {
                    if ((gMultiKill == 2) && (gAnnounceKillingSpreeTicks == 0)) // only play sfx if multi kill alert setting is on, and if there isn't a multi kill alert already active
                        sndStartSample("NOTBLOOD2", 128, -1, 22050);
...
int playerDamageSprite(int nSource, PLAYER *pPlayer, DAMAGE_TYPE nDamageType, int nDamage)
{
    ...
    else
    {
        ...
        case kDamageDrown:
            ...
            break;
        default:
            if (nHealth < -20 && ((gGameOptions.nGameType >= kGameTypeBloodBath && !(gGameOptions.uNetGameFlags&kNetGameFlagSecondWindToggle)) || (gGameOptions.nGameType == kGameTypeCoop && (gGameOptions.uNetGameFlags&kNetGameFlagSecondWindToggle))) && Chance(0x4000))
            {
                DAMAGEINFO *pDamageInfo = &damageInfo[nDamageType];
