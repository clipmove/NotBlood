...
void MyLoadSave::Save(void)
{
    ...
    char showinvisibilitytemp; // don't set to current showinvisibility var
    Write(&showinvisibilitytemp, sizeof(showinvisibilitytemp));
...
bool LoadSavedInCurrentSession(int nSlot)
{
    ...
    if (gSaveGameOptions[nSlot].bPermaDeath != gGameOptions.bPermaDeath)
        return false;
    if (gSaveGameOptions[nSlot].bDamage2X != gGameOptions.bDamage2X)
        return false;
...
void LoadUpdateSaveGame(int nSlot, int nSkill)
{
    ...
    if (gSaveGameOptions[nSlot].nDifficulty != gSaveGameOptions[nSlot].nEnemyHealth)
        ...
        nDifficulty = 5;
    else if (gSaveGameOptions[nSlot].bDamage2X)
        nDifficulty = 5;
