    ...
    switch (pSprite->statnum) {
        case kStatDude: {
            ...
            if ((damage > 0) && bWasAlive) {
                ...
                {
                    if (!pSourcePlayer->damageAccumulatedTick || (damage > 25))
        ...
        if (nXSprite > 0)
        {
            ...
            if (IsPlayerSprite(pSprite))
            {
                ...
                if (pPlayer->isUnderwater)
                {
                    ...
                    if (pPlayer->underwaterTime < 1080 && packCheckItem(pPlayer, kPackDivingSuit) && !bDivingSuit && (((pPlayer->pXSprite->health > 0) && (gAutoDivingSuit || numplayers > 1)) || VanillaMode())) // don't activate diving suit if player is dead
...
void actFireVector(spritetype *pShooter, int a2, int a3, int a4, int a5, int a6, VECTOR_TYPE vectorType)
{
    ...
    if (nRange == 0 || approxDist(gHitInfo.hitx-shooterPos.x, gHitInfo.hity-shooterPos.y) < nRange)
    {
        ...
        case 3:
        {
            ...
            if ((vectorType == kVectorTine) && pShooter && IsPlayerSprite(pShooter) && !IsPlayerSprite(pSprite) && IsDudeSprite(pSprite) && WeaponsNotBlood() && !VanillaMode()) // apply double damage for pitchfork when attacking rear of enemy
            {
                const int nDiff = (klabs(pShooter->ang-pSprite->ang)%2048+2048)%2048;
                if (nDiff < 256) // if attacker is within 45 degrees behind victim, apply double damage
                    shift++;
            }
            actDamageSprite(nShooter, pSprite, dmgType, pVectorData->dmg<<shift);
            int nXSprite = pSprite->extra;
