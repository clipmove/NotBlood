...
void SetRadialMenuToggle(CGameMenuItemZCycle *);
void SetRadialMenuPosition(CGameMenuItemSlider *);
...
void SetRadialMenuReticlePosition(CGameMenuItemZBool *);
...
void SetRadialMenuYawInvert(CGameMenuItemZBool *);
void SetRadialMenuPitch(CGameMenuItemZCycle *);
void SetRadialMenuPitchInvert(CGameMenuItemZBool *);
...
CGameMenuItemZBool itemNetGameBoolPlayerColors("PLAYER COLORS:", 3, 66, 97, 180, true, 0, "EXTENDED", "VANILLA");
CGameMenuItemZBool itemNetGameBoolFriendlyFire("FRIENDLY FIRE:", 3, 66, 105, 180, true, 0, NULL, NULL);
CGameMenuItemZCycle itemNetGameCycleKey("KEYS SETTING:", 3, 66, 113, 180, 0, 0, zKeyStrings, ARRAY_SSIZE(zKeyStrings), 0);
CGameMenuItemZCycle itemNetGameCycleItemWeapon("ITEM/WEAPON SETTING:", 3, 66, 121, 180, 0, SetNetGameMode, zKeepItemWeaponStrings, ARRAY_SSIZE(zKeepItemWeaponStrings), 0);
CGameMenuItemZBool itemNetGameBoolTeamColors("TEAM COLORS:", 3, 66, 97, 180, true, 0, NULL, NULL);
...
CGameMenuItemSlider itemOptionsDisplayPolymostRollAngle("VIEW ROLLING:", 3, 66, 170, 180, &gRollAngle, -5, 5, 1, UpdateRollAngle, -1, -1, kMenuSliderValue);
#endif
...
CGameMenu menuOptionsControlRadial;
...
void SetJoystickSnapZone(CGameMenuItemSlider* pItem);
...
const char *pzRadileMenuSoundStrings[] =
{
    "OFF",
    "SHARP",
    "MEDIUM",
...
const char *pzRadileMenuReticle[] =
{
    "OFF",
    "ON (SMALL)",
    "ON (MEDIUM)",
...
CGameMenuItemChain itemOptionsControlKeyboard("KEYBOARD SETUP", 1, 0, 50, 320, 1, &menuOptionsControlKeyboard, -1, NULL, 0);
CGameMenuItemChain itemOptionsControlMouse("MOUSE SETUP", 1, 0, 70, 320, 1, &menuOptionsControlMouse, -1, SetupMouseMenu, 0);
CGameMenuItemChain itemOptionsControlRadial("RADIAL MENU SETUP", 1, 0, 90, 320, 1, &menuOptionsControlRadial, -1, NULL, 0);
CGameMenuItemChain itemOptionsControlJoystickButtons("JOYSTICK BUTTONS SETUP", 1, 0, 130, 320, 1, &menuOptionsControlJoystickButtonAssignment[0], -1, SetupJoystickButtonsMenu, 0);
CGameMenuItemChain itemOptionsControlJoystickAxes("JOYSTICK AXES SETUP", 1, 0, 150, 320, 1, &menuOptionsControlJoystickListAxes, -1, SetupJoystickAxesMenu, 0);
CGameMenuItemChain itemOptionsControlJoystickMisc("JOYSTICK MISC SETUP", 1, 0, 170, 320, 1, &menuOptionsControlJoystickMisc, -1, NULL, 0);
...
CGameMenuItemZCycle itemOptionsControlRadialToggle("USAGE BEHAVIOR:", 3, 66, 40, 180, 0, SetRadialMenuToggle, zRadialMenuToggle, ARRAY_SIZE(zRadialMenuToggle), 0);
CGameMenuItemSlider itemOptionsControlRadialPosition("POSITION:", 3, 66, 50, 180, &gRadialMenuPosition, 0, 320, 16, SetRadialMenuPosition, -1, -1, kMenuSliderPercent);
CGameMenuItemZBool itemOptionsControlRadialDimBackground("DIM BACKGROUND:", 3, 66, 60, 180, 0, SetRadialMenuDimBackground, NULL, NULL);
...
CGameMenuItemZBool itemOptionsControlRadialReticlePosition("RETICLE POSITION:", 3, 66, 110, 180, 0, SetRadialMenuReticlePosition, "MOVING", "LOCKED");
CGameMenuItemSlider itemOptionsControlRadialMouseSensitivity("MOUSE SENSITIVTY:", 3, 66, 120, 180, &gRadialMenuMouseSensitivity, fix16_from_float(0.1f), fix16_from_float(3.f), fix16_from_float(0.1f), SetRadialMenuMouseSensitivity, -1, -1, kMenuSliderQ16);
...
CGameMenuItemZCycle itemOptionsControlRadialYaw("JOY X AXIS:", 3, 66, 150, 180, 0, SetRadialMenuYaw, zRadialMenuAxes, ARRAY_SIZE(zRadialMenuAxes), 0);
CGameMenuItemZBool itemOptionsControlRadialYawInvert("JOY X INVERT:", 3, 66, 160, 180, 0, SetRadialMenuPitchInvert, NULL, NULL);
CGameMenuItemZCycle itemOptionsControlRadialPitch("JOY Y AXIS:", 3, 66, 170, 180, 0, SetRadialMenuPitch, zRadialMenuAxes, ARRAY_SIZE(zRadialMenuAxes), 0);
CGameMenuItemZBool itemOptionsControlRadialPitchInvert("JOY Y INVERT:", 3, 66, 180, 180, 0, SetRadialMenuYawInvert, NULL, NULL);
...
void SetupDifficultyMenu(void)
{
    ...
    menuCustomDifficulty.Add(&itemCustomDifficultyStartLevel, false);
...
void SetupNetStartMenu(void)
{
    ...
    menuNetworkGameMode.Add(&itemNetGameBoolPlayerColors, false);
    ...
    itemNetGameBoolPlayerColors.tooltip_pzTextUpper = "Allow non-vanilla player colors";
...
void SetupControlsMenu(void)
{
    ...
    menuOptionsControl.Add(&itemOptionsControlRadial, false);
    ...
    menuOptionsControlRadial.Add(&itemOptionsControlRadialTitle, false);
    menuOptionsControlRadial.Add(&itemOptionsControlRadialToggle, true);
    menuOptionsControlRadial.Add(&itemOptionsControlRadialPosition, false);
    menuOptionsControlRadial.Add(&itemOptionsControlRadialDimBackground, false);
    ...
    menuOptionsControlRadial.Add(&itemOptionsControlRadialReticlePosition, false);
    ...
    menuOptionsControlRadial.Add(&itemOptionsControlRadialPitchInvert, false);
    menuOptionsControlRadial.Add(&itemBloodQAV, false);

    itemOptionsControlRadialToggle.m_nFocus = gRadialMenuToggle % ARRAY_SSIZE(zRadialMenuToggle);
    itemOptionsControlRadialPosition.nValue = gRadialMenuPosition;
    ...
    itemOptionsControlRadialReticlePosition.at20 = !!gRadialMenuReticlePosition;
...
void SetAmmoScale(CGameMenuItemZCycle *pItem)
{
    if ((gGameOptions.nGameType == kGameTypeSinglePlayer) && (numplayers == 1)) {
        gAmmoScale = pItem->m_nFocus % ARRAY_SSIZE(pzAmmoScaleStrings);
        gGameOptions.nAmmoScale = gAmmoScale;
    } else {
        pItem->m_nFocus = gAmmoScale % ARRAY_SSIZE(pzAmmoScaleStrings);
    }
}
...
void SetRadialMenuToggle(CGameMenuItemZCycle *pItem)
{
    gRadialMenuToggle = pItem->m_nFocus % ARRAY_SSIZE(zRadialMenuToggle);
}

void SetRadialMenuPosition(CGameMenuItemSlider *pItem)
{
    gRadialMenuPosition = pItem->nValue;
}

void SetRadialMenuDimBackground(CGameMenuItemZBool *pItem)
{
    gRadialMenuDimBackground = pItem->at20;
}

void SetRadialMenuDimHUD(CGameMenuItemZBool *pItem)
{
    gRadialMenuDimHud = pItem->at20;
}

void SetRadialMenuSlowDown(CGameMenuItemZBool *pItem)
{
...
{
    gRadialMenuReticle = pItem->m_nFocus % ARRAY_SIZE(pzRadileMenuReticle);
    itemOptionsControlRadialReticlePosition.bEnable = pItem->m_nFocus != 0;
}

void SetRadialMenuReticlePosition(CGameMenuItemZBool *pItem)
{
    gRadialMenuReticlePosition = pItem->at20;
}

void SetRadialMenuMouseSensitivity(CGameMenuItemSlider *pItem)
{
    gRadialMenuMouseSensitivity = pItem->nValue;
}
...
void SetRadialMenuYawInvert(CGameMenuItemZBool *pItem)
{
    gRadialMenuYawInvert = pItem->at20;
}

void SetRadialMenuPitch(CGameMenuItemZCycle *pItem)
{
    gRadialMenuPitch = pItem->m_nFocus % ARRAY_SIZE(zRadialMenuAxes);
}

void SetRadialMenuPitchInvert(CGameMenuItemZBool *pItem)
{
    gRadialMenuPitchInvert = pItem->at20;
}
...
void SetCustomDifficultyMenu(CGameMenuItemChain *pItem)
{
    ...
    else
    {
        ...
        else
        {
            ...
            itemCustomDifficultyStartLevel.nRangeHigh = pEpisodeInfo->nLevels-1;
        }
    }
    itemCustomDifficultyStartLevel.bNoDraw = !itemCustomDifficultyStartLevel.bEnable;
}
...
void SetCustomDifficultyAndStart(CGameMenuItemChain *pItem)
{
    ...
    else if (itemCustomDifficultyStartLevel.nValue > 0)
        gGameOptions.nLevel = itemCustomDifficultyStartLevel.nValue;
    gGameMenuMgr.Deactivate();
}
...
void SetupJoystickAxesMenu(CGameMenuItemChain *pItem)
{
    ...
    for (int nAxis = 0; nAxis < MAXJOYAXES; nAxis++) // set settings for each listed joystick axis
    {
        ...
        pItemOptionsControlJoystickAxisSnapZone[nAxis]->nValue = JoystickAnalogueSnap[nAxis];
    }
}
...
inline void GenerateJoystickAxisHeatmap(const int nAxis)
{
    ...
    CONTROL_GetAxisHeatMap(nAxis, (uint8_t*)waloff[kAxisHeatmapTile], 48, 48, 0, 31, true);
...
void SetJoystickSnapZone(CGameMenuItemSlider* pItem)
{
    for (int nAxis = 0; nAxis < MAXJOYAXES; nAxis++)
    {
        if (pItem == pItemOptionsControlJoystickAxisSnapZone[nAxis])
        {
...
void StartNetGame(CGameMenuItemChain *pItem)
{
    ...
    if (!itemNetGameBoolPlayerColors.at20)
        gPacketStartGame.uNetGameFlags |= kNetGameFlagVanillaColors;
